...
describe("Given an authenticated user", () => {
  ...
  describe("When he sends a tweet", () => {
    ...
    it("Saves the tweet in the tweets table", async () => {
      await then.tweet_exists_TweetsTable(tweet.id);
    });
    
    it("Saves the tweet in the timelines table", async () => {
        await then.tweet_exists_TimelinesTable(user.username, tweet.id);
    });

    it("Increments tweet count in the users table", async () => {
