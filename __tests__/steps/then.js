...
const retweet_exists_in_TweetsTable = async (userId, tweetId) => {
  ...
  console.log(`looking for tweet [${tweetId}] in table [${process.env.TWEETS_TABLE}]`)
...
const retweet_exists_in_RetweetsTable = async (userId, tweetId) => {
  ...
  console.log(`looking for retweet [${tweetId}] for user [${userId}] in table [${process.env.RETWEETS_TABLE}]`)
  const resp = await DynamoDB.get({
...
const there_are_N_tweets_in_TimelinesTable = async (userId, n) => {
  ...
  console.log(`looking for tweets for n [${n}] tweets for user [${userId}] in table [${process.env.TIMELINES_TABLE}]`)
  const resp = await DynamoDB.query({
    TableName: process.env.TIMELINES_TABLE,
